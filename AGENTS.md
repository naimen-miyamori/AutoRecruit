...
- Batch mode uses `--jobs-file` as the only source of job definitions. Do not allow it to combine with single-job `--keyword`, `--jd`, or `--jd-file`; normal run-level switches such as `--include-viewed`, `--email`, `--cc`, and valid Liepin forwarding remain allowed.
...
- Batch mode: `rtk npm run dev -- --platform <platform|all> --jobs-file ./jobs.json [--include-viewed true] [--liepin-forward-contact "<contact name>"]`
