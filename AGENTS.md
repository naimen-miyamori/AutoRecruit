...
The CLI also has a standalone search-subscription mode through `--search-subscription-file`. This mode opens the platform search-condition page, fills the keyword, leaves non-keyword conditions as explicit `skipped` results until platform filter modules are implemented, reads and reports the result total, and only saves the search condition/subscription when `--save-search-subscription true` is provided. It does not parse JD text, create job records, capture resumes, score candidates, export reports, or send email.
...
- `npm run dev -- --platform 51job --search-subscription-file ./search-subscription.json [--keyword "东南亚 销售"] [--search-subscription-name "东南亚 销售订阅"] [--save-search-subscription true]` — open the platform search-condition page, fill the keyword, report the result total, and optionally save the search condition/subscription without running JD parsing or resume capture. The JSON file contains `keyword`, optional `savedSearchName`, and optional `conditions`; `--keyword` and `--search-subscription-name` override the file values.
- `npm run dev -- --platform all --search-subscription-file ./search-subscription.json [--save-search-subscription true]` — run the standalone search-subscription mode sequentially for 51job, Liepin, then Zhilian.
...
- `src/search/search-subscription.ts` holds the standalone search-subscription orchestration. It loads the manual condition JSON, opens the adapter search-condition page, applies only implemented conditions, reads the result total, and saves only when explicitly requested.
