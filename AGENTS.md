...
- Zhilian search-subscription mode must also prepare the page through the saved quick-search tag and confirm visible `关键词：<keyword>` state. Do not replace the saved quick-search by filling the raw keyword search box. Explicit empty-result text such as `没有符合条件的人才` is a successful zero-result summary.
