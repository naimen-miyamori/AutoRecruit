...
- Liepin forces headed mode even when `PLAYWRIGHT_HEADLESS=true`. Liepin reusable browser mode is enabled by default, keeps the recruiter search browser open after runs, and is controlled by `PLAYWRIGHT_LIEPIN_REUSE_BROWSER` plus `PLAYWRIGHT_LIEPIN_REUSE_CDP_PORT`.
...
- `PLAYWRIGHT_LIEPIN_ACTION_DELAY_MIN_MS` default `2000`
- `PLAYWRIGHT_LIEPIN_ACTION_DELAY_MAX_MS` default `3000`
- `PLAYWRIGHT_LIEPIN_CANDIDATE_DELAY_MIN_MS` default `2000`
- `PLAYWRIGHT_LIEPIN_CANDIDATE_DELAY_MAX_MS` default `3000`
- `PLAYWRIGHT_LIEPIN_REUSE_BROWSER` default enabled unless set to `false`
