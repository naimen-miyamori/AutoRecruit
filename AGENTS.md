...
- Boss chat messages must be selected from the page's common-phrase panel opened through `.toolbar-icon.changyongyu`; do not fill the editor directly. For a matched candidate, keep the resume dialog open and forward first, record `forwarded: true`, close the resume, select `方便发一份你的简历过来吗？`, send it, then request phone exchange through `换电话` -> `确定`. For an unmatched candidate, close the resume, select the page's `对不起，看了你的简历以后觉得不太合适，希望你早日找到满意的工作机会`, and send it; do not forward or request phone exchange. Message and phone actions must remain idempotent.
