...
4. Wait for the talent-search page to reach a known ready state, then collect candidate IDs from the result cards. A visible empty result list is treated as a successful zero-candidate run rather than an extraction failure.
...
- A visible empty candidate list is a successful run outcome, not an extraction error; that path should still write a run result, export the latest markdown, and send the no-new-candidates email when a recipient is configured.
