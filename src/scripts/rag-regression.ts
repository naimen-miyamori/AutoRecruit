...
import { readRagRegressionSuiteFile, runRagRegression } from '../rag/regression.js';
...
async function main(): Promise<void> {
  ...
  if (!args.suiteFile) {
    throw new Error('Usage: npm run rag:regression -- --suite-file ./fixtures/rag/regression.json');
  }

  const suiteFile = path.resolve(args.suiteFile);
