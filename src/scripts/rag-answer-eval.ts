...
async function readEvalCases(filePath: string): Promise<ReturnType<typeof normalizeRagAnswerEvalCases>> {
  const payload = JSON.parse(await fs.readFile(filePath, 'utf8')) as unknown;
  return normalizeRagAnswerEvalCases(payload);
}
...
async function main(): Promise<void> {
  ...
  if (!jobKey || !args.evalFile) {
    throw new Error('Usage: npm run rag:answer-eval -- --platform <platform> --keyword "<keyword>" --eval-file ./rag-answer-eval.json');
  }

  const summary = await evaluateRagAnswers({
    platform: args.platform,
    jobKey,
    cases: await readEvalCases(args.evalFile),
    topK: args.topK,
  });
  console.log(JSON.stringify(summary, null, 2));

  if (args.failOnMismatch && summary.failedCount > 0) {
    process.exitCode = 1;
  }
}
