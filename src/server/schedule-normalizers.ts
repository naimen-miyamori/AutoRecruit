...
function normalizeKind(value: unknown): SchedulableTaskKind {
  if (value === 'resume-capture' || value === 'batch' || value === 'talent-mapping' || value === 'search-subscription' || value === 'boss-auto-chat' || value === 'boss-job-sync') {
    return value;
  }
  throw new Error('scheduled task kind must be resume-capture, batch, talent-mapping, search-subscription, boss-auto-chat, or boss-job-sync');
}
