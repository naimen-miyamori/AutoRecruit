...
import { isScheduleRunId } from './schedule-identifiers.js';
import type {
...
function normalizeKind(value: unknown, taskKey: string): SchedulableTaskKind {
  return assertRecurringScheduleTaskKind(value, taskKey);
}
...
function normalizeTaskTemplates(value: unknown): NormalizedScheduledTaskTemplate[] {
  ...
  return value.map((item, index) => {
    ...
    return {
      ...
      kind: normalizeKind(task.kind, taskKey),
      input: input as Record<string, unknown>,
    };
  });
}

export function normalizeScheduleCreate(payload: unknown, now = new Date()): NormalizedScheduleDefinition {
...
export function normalizeScheduleUpdate(
  current: PersistedScheduleDefinition,
  payload: unknown,
  now = new Date(),
  ...
  const currentStatus = current.status === 'enabled' || current.status === 'paused'
    || current.status === 'stop_requested' || current.status === 'stopped'
    ? current.status
    : 'paused';
  const currentCreatedAt = typeof current.createdAt === 'string' && Number.isFinite(Date.parse(current.createdAt))
    ? current.createdAt
    : now.toISOString();
  const optionalTimestamp = (value: unknown): string | undefined => typeof value === 'string'
    && Number.isFinite(Date.parse(value))
    ? value
    : undefined;
  return {
    ...normalized,
    scheduleId: expectedScheduleId ?? current.scheduleId,
    status: currentStatus,
    createdAt: currentCreatedAt,
    updatedAt: now.toISOString(),
    stopRequestedAt: optionalTimestamp(current.stopRequestedAt),
    activeRunId: typeof current.activeRunId === 'string' && isScheduleRunId(current.activeRunId)
      ? current.activeRunId
      : undefined,
    nextRunAt: optionalTimestamp(current.nextRunAt),
    lastRunAt: optionalTimestamp(current.lastRunAt),
    consecutiveFailures: typeof current.consecutiveFailures === 'number'
      && Number.isSafeInteger(current.consecutiveFailures)
