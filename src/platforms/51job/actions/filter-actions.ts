...
async function clickLocatorWithMouse(page: Page, locator: Locator): Promise<void> {
  await click51jobControl(
    create51jobActionContext(page, 3000, 'control click'),
    locator,
  );
}

async function clickLocatorWithDomEvents(page: Page, locator: Locator): Promise<void> {
  await click51jobControlWithDomEvents(
    create51jobActionContext(page, 3000, 'DOM compatibility click'),
    locator,
  );
}
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      if (attempt === 0) {
        ...
        await clickLocatorWithDomEvents(page, trigger);
      }
...
async function apply51jobElementSelectApplicationFilter(
  ...
  await clickLocatorWithMouse(page, triggerInput).catch(async () => clickLocatorWithDomEvents(page, triggerInput));
...
async function apply51jobDynamicTextInputApplicationFilter(
  ...
  const trigger = await findPreferred51jobLocatorBySelectors(page, [target.triggerSelector], 'true')
    ?? page.locator(target.triggerSelector).first();
  await trigger.waitFor({ state: 'visible', timeout: 3000 });
  await clickLocatorWithMouse(page, trigger).catch(async () => clickLocatorWithDomEvents(page, trigger));
...
export async function apply51jobSearchCondition(
  page: Page,
...
export async function discover51jobSearchFilters(
  page: Page,
  options: SearchFilterDiscoveryRunOptions,
