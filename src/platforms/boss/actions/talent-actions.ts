...
export interface BossTalentGreetActionInput {
  source: Exclude<BossTalentSource, 'normal-search'>;
  candidateId: string;
...
async function resizeRequirementRows(page: Page, group: Locator, desiredCount: number, title: string): Promise<void> {
  ...
  while (count < desiredCount) {
    ...
    await clickBossControl(add, page, config.playwright.resumeDetailTimeoutMs);
  ...
  while (count > desiredCount) {
    ...
    await clickBossControl(remove, page, config.playwright.resumeDetailTimeoutMs);
    rows = group.locator('.form-content-list-item');
    const nextCount = await rows.count();
    if (nextCount >= count) {
...
async function synchronizeRequirementGroup(
  ...
  for (let index = 0; index < desiredValues.length; index += 1) {
    ...
    if (!await editor.isVisible().catch(() => false)) {
      await clickBossControl(row.locator('.form-content-word').first(), page, config.playwright.resumeDetailTimeoutMs);
    }
...
export async function triggerBossDeepSearchMatch(page: Page): Promise<BossTalentCandidate[]> {
  ...
  await clickBossControl(button, page, config.playwright.searchPageTimeoutMs);
...
export async function greetBossTalentCandidateAction(
  page: Page,
  input: BossTalentGreetActionInput,
): Promise<BossGreetResult> {
  await openBossTalentPage(page, input.source);
  const root = await resolveTalentRoot(page, input.source);
  const index = await resolveCardIndex(root, input.candidateId);
  if (index < 0) {
    throw new Error(`Boss candidate ${input.candidateId} is no longer present on the ${input.source} page.`);
  }
  ...
  await clickBossControl(chat, page, config.playwright.resumeDetailTimeoutMs);
