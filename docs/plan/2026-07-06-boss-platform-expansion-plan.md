...
- 岗位 JD、报告邮箱、搜索来源、direct 原始筛选输入、标准化搜索条件和 Boss 岗位转发目标统一保存在 `data/<platform>/jobs/<jobKey>/jd.json`。
- 首次显式配置后，普通抓取复跑可只传 `--platform` 和 `--keyword`；系统复用岗位配置，不再依赖原筛选输入文件。
- Boss 自动聊天的最近转发和总结邮件配置保存在 `data/boss/chat-review/automation-settings.json`。未显式传参时，转发优先读岗位配置，再读平台默认配置。
- 配置采用单值覆盖和邮箱列表去重，不保存重复历史项；内容未变化时不重写文件。
- 缺少可用转发配置时，不打开对应未读会话，不进行转发、聊天或换电话。
...
- Web 控制台“提交任务”增加“Boss 自动沟通”，平台固定为 Boss，分数线默认 70；转发和总结邮件可显式提供，也可复用已保存配置。
...
- 本机 `data/boss/jobs/物业电工/jd.json` 初始严格要求为：`47岁以下，高低压证、物业行业电工经验、一家公司工作至少2年以上`；后续阶段 10 增加上海人要求和受控确认分支。
...
- 任一条件没有明确简历证据时按不符合处理，不进行推测；阶段 10 为上海人条件增加了不直接推断、只发送问题确认的受控例外。
...
- 物业电工 JD 增加第六项硬要求 `上海人`。现居地或期望城市为上海不算籍贯证据，只有明确籍贯字段、`上海人`、`上海户籍`、`沪籍` 等文本才可直接满足。
- 简历未明确上海或非上海籍贯时检查教育经历。学校名称或教育详情含上海学校线索，只能用于发起确认，不能直接判定为上海人。
- 只有年龄、双证、物业电工经历和两年任职五项全部满足时才发起上海籍确认；其他条件已经不符合时直接走原不合适流程。
