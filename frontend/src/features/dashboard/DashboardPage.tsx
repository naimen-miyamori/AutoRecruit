...
export function DashboardPage() {
  ...
  const runtimesQuery = useQuery({ queryKey: queryKeys.platformBrowserRuntimes, queryFn: ({ signal }) => api.listPlatformBrowserRuntimes(signal), refetchInterval: 15_000 });
  const refresh = () => void Promise.all([tasksQuery.refetch(), healthQuery.refetch(), jobsQuery.refetch(), runtimesQuery.refetch()]);
  ...
  return (
    <div className="page-stack">
      ...
      {(tasksQuery.error || healthQuery.error || jobsQuery.error || runtimesQuery.error) && <ErrorState error={tasksQuery.error ?? healthQuery.error ?? jobsQuery.error ?? runtimesQuery.error} onRetry={refresh} />}
      ...
      {runtimesQuery.data && <Section title="登录持有浏览器运行时" description="这里只读展示清单、租约和 CDP 可达性；任务执行时仍会在租约内重新验证登录和页面身份。"><div className="table-wrap"><table><thead><tr><th>平台</th><th>状态</th><th>Generation</th><th>修订</th><th>占用操作</th><th>发布时间</th></tr></thead><tbody>{runtimesQuery.data.runtimes.map((runtime) => { const ok = runtime.status === 'published'; const warning = runtime.status === 'busy' || runtime.status === 'degraded' || runtime.status === 'starting' || runtime.status === 'login_required' || runtime.status === 'absent'; return <tr key={runtime.platform}><td><span className={`platform-mark platform-${runtime.platform}`}>{PLATFORM_LABELS[runtime.platform]}</span></td><td><StatusPill status={ok ? 'ok' : warning ? 'warning' : 'failed'} label={runtime.status} /></td><td>{runtime.generationFingerprint ?? '—'}</td><td>{runtime.revision ?? '—'}</td><td>{runtime.occupiedBy ? `${runtime.occupiedBy.operationKind} · ${runtime.occupiedBy.operationId}` : '空闲'}</td><td>{formatCompactDate(runtime.publishedAt)}</td></tr>; })}</tbody></table></div></Section>}
