  ...
  return (
    <div className="page-stack">
      ...
      <Section title="完整运行历史" description={`${runsQuery.data?.runs.length ?? 0} 次运行`}>
        ...
        <div className="table-wrap"><table><thead><tr><th>时间</th><th>页面搜索词</th><th>总候选</th><th>新增</th><th>已评分</th><th>失败</th><th>结果文件</th></tr></thead><tbody>{runsQuery.data?.runs.map((run) => <tr key={`${run.fetchedAt}-${run.resultFile ?? ''}`}><td>{formatDate(run.fetchedAt)}</td><td>{run.searchExecution?.pageKeyword ?? '-'}</td><td>{run.totalCandidates}</td><td>{run.newCandidateIds.length}</td><td>{run.scoredCandidates.length}</td><td><StatusPill status={run.failedCandidates.length ? 'failed' : 'succeeded'} label={String(run.failedCandidates.length)} /></td><td className="mono">{run.resultFile ?? '-'}</td></tr>)}</tbody></table></div>
