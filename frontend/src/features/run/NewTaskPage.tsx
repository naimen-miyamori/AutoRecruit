...
interface FormState {
  ...
  resultRoutingChoice: ResultRoutingChoice;
...
function targetPlatforms(form: FormState, mode: Mode): Platform[] {
  ...
  if (form.includeBoss && (mode === 'resume-capture' || mode === 'batch' || mode === 'search-subscription')) platforms.push('boss');
  return platforms;
}
...
export function NewTaskPage() {
  ...
  const showResultRoutingSettings = (mode === 'resume-capture' || mode === 'batch') && form.platform !== 'boss';
  ...
  const submit = () => {
    ...
    if (mode === 'resume-capture') {
      ...
      if (!form.searchSubscriptionFile.trim()) return setValidationError('订阅管理文件必填');
      body = {
    ...
    <div className="page-stack">
      ...
      <Section title="任务参数" description={mode === 'boss-auto-chat' ? 'Boss 是单平台独立模式。' : mode === 'talent-mapping' ? '此入口只执行卡片扫描；详情补全需在人才地图项目页核对精确人数并逐轮确认。' : mode === 'search-subscription' ? '订阅管理只选择或保存平台原生订阅，不抓取候选、不打开详情、不写 seen/评分/报告；全部平台默认不包含 Boss。' : '全部主平台默认运行 51job、猎聘和智联；普通抓取与批量可显式追加直猎邦。'}>
        <div className="form-grid">
          ...
          {showBossCaptureSettings && <><label className="wide"><span>Boss 评分后模型要求分流</span><select value={form.bossScreeningChoice} onChange={(event) => setBossScreeningChoice(event.target.value as BossScreeningChoice)}><option value="inherit">复用岗位设置（不写回）</option><option value="enabled">本次启用并保存</option><option value="disabled">本次停用并保存</option></select></label><label className="checkbox-field wide"><input type="checkbox" checked={form.bossScreeningChoice === 'enabled'} onChange={(event) => setBossScreeningChoice(event.target.checked ? 'enabled' : 'disabled')} />启用 Boss 评分后模型要求分流</label><small className="wide">“复用岗位设置”不会覆盖岗位配置；选择启用或停用会生成明确的岗位写回意图，避免把未勾选误当成继承。</small>{form.bossScreeningEnabled === true && <><div className="security-note wide">候选人会先评分：明确满足和需复核者转发给主受众；明确否定简历逐份邮件发给副收件人，正文包含否定原因和完整简历；模型明确判断要求缺失者不做 Boss 转发。</div><label className="wide"><span>模型要求策略文件（可选，留空复用岗位已保存策略）</span><input value={form.bossScreeningPolicyFile} onChange={(event) => set('bossScreeningPolicyFile', event.target.value)} placeholder="./boss-model-requirements.json" /></label><label><span>否定简历副收件人</span><input type="email" value={form.bossSecondaryEmail} onChange={(event) => set('bossSecondaryEmail', event.target.value)} /></label><label><span>否定简历邮件抄送</span><input value={form.bossSecondaryCc} onChange={(event) => set('bossSecondaryCc', event.target.value)} placeholder="逗号分隔" disabled={form.clearBossSecondaryCc} /></label><label className="checkbox-field"><input type="checkbox" checked={form.clearBossSecondaryCc} onChange={(event) => set('clearBossSecondaryCc', event.target.checked)} />清除否定邮件已保存抄送</label></>}</>}
...
function commonBody(form: FormState, mode: 'resume-capture' | 'batch', compiledSearchSource?: 'saved' | 'direct'): Record<string, unknown> {
  ...
  return {
    ...
    resultRoutingEnabled: form.platform !== 'boss' ? (form.resultRoutingChoice === 'inherit' ? undefined : form.resultRoutingChoice === 'enabled') : undefined,
    resultRoutingPolicyFile: form.platform !== 'boss' ? form.resultRoutingPolicyFile.trim() || undefined : undefined,
    secondaryEmail: form.platform !== 'boss' ? form.secondaryEmail.trim() || undefined : undefined,
    secondaryCc: form.platform !== 'boss' ? splitList(form.secondaryCc) : undefined,
  };
}
