...
type Mode = 'resume-capture' | 'batch' | 'search-subscription' | 'boss-auto-chat' | 'login-refresh';
...
const initialForm: FormState = {
  platform: '51job', keyword: '', jd: '', jdFile: '', jobsFile: '', searchSubscriptionFile: '', searchSubscriptionName: '', saveSearchSubscription: false,
...
export function NewTaskPage() {
  const [mode, setMode] = useState<Mode>('resume-capture');
  ...
  return (
    <div className="page-stack">
      ...
      <Section>
        <div className="segmented">{([['resume-capture', '简历抓取'], ['batch', '批量任务'], ['search-subscription', '搜索订阅'], ['boss-auto-chat', 'Boss 自动沟通'], ['login-refresh', '登录刷新']] as const).map(([value, label]) => <button type="button" className={mode === value ? 'active' : ''} key={value} onClick={() => setModeSafe(value)}>{label}</button>)}</div>
      </Section>
      <Section title="任务参数" description={mode === 'boss-auto-chat' ? 'Boss 是单平台独立模式。' : '全部平台只包含 51job、猎聘和智联，不包含 Boss。'}>
        <div className="form-grid">
          <label><span>平台</span><select value={form.platform} onChange={(event) => set('platform', event.target.value as PlatformSelection)} disabled={mode === 'boss-auto-chat'}>{(mode === 'boss-auto-chat' ? ['boss'] : mode === 'login-refresh' ? ['51job', 'liepin', 'zhilian', 'boss'] : ['51job', 'liepin', 'zhilian', 'boss', 'all']).map((item) => <option value={item} key={item}>{PLATFORM_LABELS[item as keyof typeof PLATFORM_LABELS]}</option>)}</select></label>
        ...
        <div className="form-actions"><button className="primary-button" type="button" disabled={mutation.isPending} onClick={submit}><Play size={16} />{mutation.isPending ? '提交中' : '提交任务'}</button></div>
      </Section>
