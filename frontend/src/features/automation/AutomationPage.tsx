...
export function AutomationPage() {
  ...
  const create = () => {
    ...
    if ((form.kind === 'batch' || form.kind === 'search-subscription') && !form.file.trim()) return setFormError('该计划任务需要文件路径');
  ...
  return (
    <div className="page-stack">
      ...
      <Section title="创建计划" description="先创建为暂停状态，核对后再显式启用。">
        <div className="form-grid"><label><span>计划名称</span><input value={form.name} onChange={(event) => set('name', event.target.value)} /></label><label><span>开始时间</span><input type="time" value={form.start} onChange={(event) => set('start', event.target.value)} /></label><label><span>结束时间</span><input type="time" value={form.end} onChange={(event) => set('end', event.target.value)} /></label><label><span>任务类型</span><select value={form.kind} onChange={(event) => { const kind = event.target.value as SchedulableTaskKind; set('kind', kind); if (kind.startsWith('boss-')) set('platform', 'boss'); }}><option value="resume-capture">简历抓取</option><option value="batch">批量任务</option><option value="search-subscription">搜索订阅</option><option value="boss-auto-chat">Boss 自动沟通</option><option value="boss-job-sync">Boss 职位同步</option></select></label><label><span>平台</span><select value={form.platform} disabled={form.kind.startsWith('boss-')} onChange={(event) => set('platform', event.target.value)}><option value="51job">51job</option><option value="liepin">猎聘</option><option value="zhilian">智联</option><option value="boss">Boss</option><option value="all">全部主平台</option></select></label>{form.kind === 'resume-capture' && <label><span>关键词</span><input value={form.keyword} onChange={(event) => set('keyword', event.target.value)} /></label>}{(form.kind === 'batch' || form.kind === 'search-subscription') && <label><span>{form.kind === 'batch' ? 'Jobs 文件' : '订阅文件'}</span><input value={form.file} onChange={(event) => set('file', event.target.value)} /></label>}<label><span>完成后延迟（秒）</span><input type="number" min="0" value={form.delaySeconds} onChange={(event) => set('delaySeconds', event.target.value)} /></label><label><span>失败后延迟（秒）</span><input type="number" min="1" value={form.failureDelaySeconds} onChange={(event) => set('failureDelaySeconds', event.target.value)} /></label><label><span>失败策略</span><select value={form.failurePolicy} onChange={(event) => set('failurePolicy', event.target.value as ScheduleForm['failurePolicy'])}><option value="stop-round">停止本轮</option><option value="continue">继续后续任务</option></select></label></div>
