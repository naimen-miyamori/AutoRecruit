...
const initialForm: ScheduleForm = { name: '', start: '09:00', end: '18:00', delaySeconds: '1800', failureDelaySeconds: '300', failurePolicy: 'stop-round', pauseAfterConsecutiveFailures: '3', kind: 'resume-capture', platform: '51job', includeBoss: false, keyword: '', file: '' };
...
export function AutomationPage() {
  ...
  const taskInput = (): Record<string, unknown> => {
    if (form.kind === 'resume-capture') return { platform: form.platform, includeBoss: form.platform === 'all' ? form.includeBoss : undefined, keyword: form.keyword };
    if (form.kind === 'batch') return { platform: form.platform, includeBoss: form.platform === 'all' ? form.includeBoss : undefined, jobsFile: form.file };
    if (form.kind === 'talent-mapping') return { platform: form.platform, talentMappingFile: form.file, mappingStage: 'scan' };
    if (form.kind === 'search-subscription') return { platform: form.platform, includeBoss: form.platform === 'all' ? form.includeBoss : undefined, searchSubscriptionFile: form.file };
  ...
  return (
    <div className="page-stack">
      ...
      <Section title="创建计划" description={form.kind === 'search-subscription' ? '先创建为暂停状态；订阅管理只选择或保存平台原生订阅，不抓取候选、不打开详情。全部平台默认不包含 Boss。' : '先创建为暂停状态，核对后再显式启用。Talent Mapping 调度只接受 card-only 计划和 scan 阶段。'}>
        <div className="form-grid"><label><span>计划名称</span><input value={form.name} onChange={(event) => set('name', event.target.value)} /></label><label><span>开始时间</span><input type="time" value={form.start} onChange={(event) => set('start', event.target.value)} /></label><label><span>结束时间</span><input type="time" value={form.end} onChange={(event) => set('end', event.target.value)} /></label><label><span>任务类型</span><select value={form.kind} onChange={(event) => { const kind = event.target.value as SchedulableTaskKind; set('kind', kind); if (kind.startsWith('boss-')) set('platform', 'boss'); if (kind === 'talent-mapping' && form.platform === 'boss') set('platform', '51job'); }}><option value="resume-capture">简历抓取</option><option value="batch">批量任务</option><option value="talent-mapping">人才地图 card-only 扫描</option><option value="search-subscription">订阅管理</option><option value="boss-auto-chat">Boss 自动沟通</option><option value="boss-job-sync">Boss 职位同步</option></select></label><label><span>平台</span><select value={form.platform} disabled={form.kind.startsWith('boss-')} onChange={(event) => set('platform', event.target.value)}><option value="51job">51job</option><option value="liepin">猎聘</option><option value="zhilian">智联</option><option value="boss" disabled={form.kind === 'talent-mapping'}>Boss</option><option value="all">全部主平台</option></select></label>{form.platform === 'all' && (form.kind === 'resume-capture' || form.kind === 'batch' || form.kind === 'search-subscription') && <label className="checkbox-field"><input type="checkbox" checked={form.includeBoss} onChange={(event) => set('includeBoss', event.target.checked)} />{form.kind === 'search-subscription' ? '订阅管理包含 Boss 第 4 阶段' : '包含 Boss 直聘·直猎邦 Pro'}</label>}{form.platform === 'all' && form.kind === 'search-subscription' && form.includeBoss && <div className="security-note wide">Boss 仅选择或保存“我的订阅”；不会抓取候选、打开详情、写 seen/评分/报告或发送邮件。保存/改名会改变 Boss 平台订阅状态。</div>}{form.kind === 'resume-capture' && <label><span>关键词</span><input value={form.keyword} onChange={(event) => set('keyword', event.target.value)} /></label>}{(form.kind === 'batch' || form.kind === 'search-subscription' || form.kind === 'talent-mapping') && <label><span>{form.kind === 'batch' ? 'Jobs 文件' : form.kind === 'search-subscription' ? '订阅文件' : 'card-only Mapping 计划文件'}</span><input value={form.file} onChange={(event) => set('file', event.target.value)} /></label>}<label><span>完成后延迟（秒）</span><input type="number" min="0" value={form.delaySeconds} onChange={(event) => set('delaySeconds', event.target.value)} /></label><label><span>失败后延迟（秒）</span><input type="number" min="1" value={form.failureDelaySeconds} onChange={(event) => set('failureDelaySeconds', event.target.value)} /></label><label><span>失败策略</span><select value={form.failurePolicy} onChange={(event) => set('failurePolicy', event.target.value as ScheduleForm['failurePolicy'])}><option value="stop-round">停止本轮</option><option value="continue">继续后续任务</option></select></label></div>
