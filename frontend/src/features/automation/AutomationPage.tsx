...
import type { CliSearchModeId, Platform, SchedulableTaskKind } from '../../api/contracts';
...
const INDEPENDENT_SCHEDULE_KINDS = ['talent-mapping', 'boss-auto-chat', 'boss-job-sync'] as const satisfies readonly SchedulableTaskKind[];
...
export function AutomationPage() {
  ...
  const unsafeSubscriptionTasks = scheduleQuery.data?.tasks.filter((task) => task.kind === 'search-subscription'
  ...
  const taskInput = (): Record<string, unknown> => {
    ...
    if (selection.type === 'independent' && selection.kind === 'boss-auto-chat') return { platform: 'boss', scoreThreshold: 70, requireAllHardRequirements: true };
  ...
  return (
    <div className="page-stack">
      ...
      <Section title="独立计划类型" description="人才地图、Boss 自动沟通和 Boss 职位同步不依赖搜索模式目录。">
